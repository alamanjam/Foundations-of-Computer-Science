...
public class BumperPanel extends JPanel
{
   ...
   public BumperPanel()
   {
      ...
      bumper=new Bumper(200,200,100,150, Color.BLUE);
   ...
   public void jumpAll()
   {
    ...
    bumper.jump(FRAME, FRAME); 
    prize.jump(FRAME, FRAME);
    this.ball.setdx(Math.random() * 12.0D - 6.0D);
    this.ball.setdy(Math.random() * 12.0D - 6.0D);
    while (this.bumper.inBumper(this.ball)) {
      this.ball.jump(FRAME, FRAME);
    }
    while (this.bumper.inBumper(this.prize)) {
      this.prize.jump(FRAME, FRAME);
    }
    stepAnimation();
